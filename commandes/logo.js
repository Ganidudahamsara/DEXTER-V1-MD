...
zokou({ nomCom: "hacker",
    ...
    if (!arg || arg == "") {
        repondre("*__Exemple : * " + prefixe + "Gagana LOGO");
        return;
    }
    try {
      ...
      //  let lien = "https://e1.yotools.net" + res;
        repondre("*processing...*");
        await zk.sendMessage(origineMessage, { image: { url:anu.image}, caption: "\t *Logo by Gagana-MD*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "dragonball", categorie: "Logo", reaction: "🐉" }, async (dest, zk, commandeOptions) => {
    ...
    try {
       ...
       // var idImg = Object.values(imgInfo)[3];
        ...
        await zk.sendMessage(dest, { image: { url: imgInfo.image }, caption: " \t *Logo by Gagana-MD*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "naruto", categorie: "Logo", reaction: "⛩" }, async (dest, zk, commandeOptions) => {
    ...
    try {
        if (!arg || arg == '') {
            repondre("*_Exemple : * " + prefixe + "naruto Gagana-MD");
            return;
        }
        ...
        await zk.sendMessage(dest, { image: { url: img.image }, caption: "\t *Logo by Gagana-MD*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({nomCom: "summer", categorie: "Logo", reaction: "🌞"}, async (dest, zk, commandeOptions) => {
  ...
  mumaker.textpro("https://textpro.me/create-sunset-light-text-effects-online-for-free-1124.html", text)
    .then((data) => {
      zk.sendMessage(dest, { image: { url: data.image }, caption: 'Logo by *premium Gagana-MD*' }, { quoted: ms });
    })
    .catch(console.error);
});
...
zokou({nomCom: "neonlight", categorie: "Logo", reaction: "💡"}, async (dest, zk, commandeOptions) => {
  ...
  try{
    ...
    /*.then((data) => {*/
     zk.sendMessage(dest, { image: { url: maker.image }, caption: 'Logo by *Gagana-MD*' }, { quoted: ms });
...
zokou({nomCom: "devil", categorie: "Logo", reaction: "😈"}, async (dest, zk, commandeOptions) => {
  ...
  mumaker.textpro("https://textpro.me/create-neon-devil-wings-text-effect-online-free-1014.html",arg)
    .then((data) => {
      zk.sendMessage(dest, { image: { url: data.image }, caption: 'Logo by *Gagana-MD*' }, { quoted: ms });
    })
    .catch(console.error);
});
////////////////////
  ...
  {nomCom:"boom",categorie:
    ...
    let {ms,repondre,prefixe,arg}=commandeOptions;
        ...
        if(!arg||arg=="")
        {
          ...
          repondre(`  Exemple :* ${prefixe}boom Gagana-MD`)
          return ;
        }
       try{
              var radio2 = "e0723d60-fc0d-421f-bf8f-a9b9b61e4be6";
...
repondre("  processing ...")
         await zk.sendMessage(dest,{image:{url:img.image},caption:"\t *Logo by Gagana-MD*"})
         
        // await zk.sendMessage(dest,{image:{url:lienImage},caption:"\t *Logo by Gagana-Md*"})
...
zokou({nomCom:"purple",categorie:"Logo",reaction:"🧳"},async(dest,zk,commandeOptions)=>{
  var {ms,repondre,prefixe,arg}=commandeOptions;
    ...
    try{
      if(!arg||arg=="")
  {
    repondre(prefixe+"purple Gagana");return;}
...
zokou({nomCom:"gold",categorie:"Logo",reaction:"🧚🏿‍♀️"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"gold Gagana-MD");return;
      }
...
zokou({nomCom:"arena",categorie:"Logo",reaction:"🥵"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"arena Gagana-MD");return;
      }
...
zokou({nomCom:"incandescent",categorie:"Logo",reaction:"😋"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"incandescent Gagana-MD");return;
      }
...
zokou({nomCom:"child",categorie:"Logo",reaction:"😇"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"child Gagana-MD");return;
      }
...
zokou({nomCom:"typo",categorie:"Logo",reaction:"😇"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"typo Gagana-MD");return;
      }
...
zokou({nomCom:"light",categorie:"Logo",reaction:"🌠"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"light Gagana-MD");return;
      }
...
zokou({nomCom:"steel",categorie:"Logo",reaction:"🌠"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"steel Gagana-MD");return;
      }
...
zokou({nomCom:"sunlight",categorie:"Logo",reaction:"🌠"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"sunlight Gagana-MD");return;
      }
